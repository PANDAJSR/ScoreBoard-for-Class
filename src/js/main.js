...
// 首先设置编码，避免中文乱码
const { initPowerShellEncoding } = require('./encoding-fix.js');
const encodingResult = initPowerShellEncoding();

// 输出编码修复结果
if (encodingResult.fixed) {
  console.log(`编码修复完成：${encodingResult.originalEncoding} -> ${encodingResult.currentEncoding}`);
} else {
  console.log(`当前系统无需编码修复：${encodingResult.currentEncoding}`);
}
